...
.factory('grader', function(){
  ...
  service.determineGrade = function(values) {
    ...
    const averageValue = _.reduce(values, function (memo, val) {
        return memo + val;
      }, 0) / values.length;
...
.factory('pokedex', function(){
  ...
  _.each(pokemon, function (pokeData) {
    ...
    lookup.byStamina[service.boundKeyForValue(pokeData.stamina)].push(pokeData);

    lookup.byAttack.all[pokeData.attack] = lookup.byAttack.all[pokeData.attack] || [];
    lookup.byAttack.all[pokeData.attack].push(pokeData);
    lookup.byAttack[service.boundKeyForValue(pokeData.attack)].push(pokeData);
    ...
    lookup.byDefense[service.boundKeyForValue(pokeData.defense)].push(pokeData);
  });
...
.factory('ivCalculator',['grader','levelUpData', 'pokedex', function(grader, levelUpData, pokedex){
